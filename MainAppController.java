...
public class MainAppController {
    ...
    private void addTask() {
        try {
            ...
            if (dueDatePicker.getValue().isBefore(LocalDate.now())) {
                ...
                throw new InvalidDateException("Tanggal jatuh tempo tak boleh di masa lalu");
            }
    ...
    private void markTaskComplete() {
        try {
            ...
            if (selected == null) {
                // TODO: Throw new TaskNotSelectedException dengan pesan yang sesuai 
                throw new TaskNotSelectedException (message:"Tidak ada tugas yang dipilih untuk ditandai selesai")
            }

            // TODO: Ubah status task menjadi "Complete"
            // hint: gunakan function setStatus dari kelas Task
            selected.setStatus(status:"complete");
            taskTable.refresh();
            TaskFileManager.saveTasks(new ArrayList<>(taskList));
        }

        // TODO: Catch TaskNotSelectedException dan panggil showError sekali lagi
        catch(TaskNotSelectedException){
            showError(e.getMessage());
        }
    ...
    public static class MissingFieldException extends Exeption {
    ...
    public static class TaskNotSelectedException extends Exeption {
        public TaskNotSelectedException (String message) {
            super(message);
        }
    // TODO: Buat kelas InvalidDateException
    public static class InvalidDateException extends Exeption {
        public InvalidDateException (String message){
            super(message);
        }
    // TODO: Buat kelas MissingDateException
    public static class MissingDateException extends Exeption {
